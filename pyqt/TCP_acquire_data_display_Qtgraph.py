...
class App(QTabWidget):
    ...
    def __init__(self,st):
        ...
        self.machine_num.setFixedWidth(90)
        ...
        self.chain_num.setFixedWidth(90)
        ...
        self.para_num.setFixedWidth(90)
        ...
        self.name_num.setFixedWidth(90)
        ...
        self.push_button.setFixedWidth(90)

        self.tabBar_layout = QVBoxLayout()
                    ...
                    if self.recv_msg:
                        ...
                        if len(self.msg_list) == 723:
                            try:
                                ...
                                ##将数据存入数据库
                                try:
                                    self.conn_insert = pymysql.connect(host='localhost', port=3306, user='root', password='mysql',
                                                                database='resistance')
    ...
    def mysql_server_concurrency(self):
        ...
        self.conn = pymysql.connect(host='localhost', port=3306, user='root', password='mysql', database='resistance')
    ...
    def mysql_last_point(self):
        ...
        self.cs1.execute("select * from {}_{} where id = {}".format(datetime.datetime.now().year, datetime.datetime.now().month,
                                                       self.last_num[0][0]))
        # self.cs1.execute('select * from 2020_7')
        self.last_data = self.cs1.fetchall()
        # self.cs1.execute("select max(id) from 2020_7")
        # self.last_num = self.cs1.fetchall()
        # self.cs1.execute("select * from 2020_7 where id ={}".format(self.last_num[0][0]))
        # self.last_data = self.cs1.fetchall()
