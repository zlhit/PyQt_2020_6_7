            ...
            for client, address in self.client_socket_list:
                try:
                    ...
                    if self.recv_msg:
                        ...
                        if len(data_list_) > 20:
                            if data_list_[0]+ data_list_[1]+data_list_[2] == 'aabbcc':
                                # 取出半波数
                                self.phase_num = int(self.msg_list[3], 16)
                                self.phase_num_list = [(int(self.current_rms_str_list[i + 5] + self.current_rms_str_list[i + 4], 16))
                                                         for i in range(0, self.phase_num * 2, 2)]
                        else:
                            ##不是第一次拿到
                    ...
                    if recv_msg:
                        ...
                        if len(self.data_instance) == 308000:
                            ...
                            self.point_instance_list = self.data_instance[
                                                       3 * 100000 + 6 * 1000: 3 * 100000 + 6 * 1000 + 1 * 2000]
                            self.point_instance = [
