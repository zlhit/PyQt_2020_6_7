...
class character_data:
    ...
    def tcp_server_start(self):
        ...
        try:
            ...
            #绑定udp发送数据
            # self.udp_socket.bind((self.ip,9990))
                ...
                try:
                    ...
                    if self.recv_msg:
                       ...
                       # print(len(self.msg_list))
                        ...
                        if len(msg_list) > 20:
                            if msg_list[0]+ msg_list[1]+msg_list[2] == 'aabbcc':
                                # 取出半波数
                                self.msg_list = []
                                 # 取出半波数
                                self.phase_num_show = int(data_list_[3], 16)
                                print('周波数{}'.format(self.phase_num))

                                self.phase_num = 3

                                self.phase_num_list = [(int(data_list_[i + 1] + data_list_[i], 16))
                                                         for i in range(4, self.phase_num * 2 + 4, 2)]
                                print(self.phase_num_list)
                                self.halfwave_num = sum(self.phase_num_list)
                        else:

                            self.phase_num = 3
                            self.halfwave_num = 500

                        

                        print(self.msg_list)
                        print(len(self.msg_list))
                        print(self.halfwave_num * 56 + self.phase_num * 2 + 4)

                        if len(self.msg_list) > self.halfwave_num * 56 + self.phase_num * 2 + 4 + 1:
                            self.msg_list = []

                        # 如果收集到的数达到指定数量，开始继续处理
                        if len(self.msg_list) == self.halfwave_num * 56 + self.phase_num * 2 + 4:
                            self.msg_list_sub = self.msg_list[self.phase_num * 2 + 4:]
                            print(len(self.msg_list))
                            ...
                            try:
                                ...
                                #电流有效值
                                self.current_rms_list = []
                                # 取出电压有效值
                                self.voltage_rms_list = []

                                #电阻值
                                self.resistance_list = []

                                for point_index,phase_num_sub in enumerate(self.phase_num_list):
                                	##取出该段数对应的数据
                                    # data_of_this_point = self.msg_list_sub[phase_num_sub * 14*point_index : phase_num_sub * 14 * (point_index+1)]
									data_of_this_point = self.msg_list_sub[ : phase_num_sub * 14*point_index]
									self.msg_list_sub = self.msg_list_sub[phase_num_sub * 14*point_index : ]



									# 取出电流有效值
                                    self.current_rms_str_list = data_of_this_point[:phase_num_sub * 4]

                                    self.current_rms_list.append([(int(
                                        ...
                                        self.current_rms_str_list[i + 1] + self.current_rms_str_list[i], 16)) ** 0.5
                                                             for i in range(0, hase_num_sub * 4, 4)])
                                    # 取出电压有效值
                                    self.voltage_rms_str_list = data_of_this_point[phase_num_sub * 4 : phase_num_sub * 4 * 2]

                                    self.voltage_rms_list.append([(int(
                                        ...
                                        self.voltage_rms_str_list[i + 1] + self.voltage_rms_str_list[i], 16)) ** 0.5
                                                             for i in range(0, phase_num_sub * 4, 4)])
                                    # 取出电流平均值
                                    self.current_mean_str_list = data_of_this_point[phase_num_sub * 4 * 2 : phase_num_sub * 4 * 2 + phase_num_sub * 2]

                                    self.current_mean_list = [
                                        int(
                                            self.current_mean_str_list[i + 1] + self.current_mean_str_list[i], 16)
                                        for i in range(0, phase_num_sub * 2, 2)]

                                    # 取出电压平均值
                                    self.voltage_mean_str_list = data_of_this_point[phase_num_sub * 4 * 2 + phase_num_sub * 2 : phase_num_sub * 4 * 2 + phase_num_sub * 2 * 2]
                                    ...
                                    # 取出电流微分平均值
                                    self.current_diff_mean_str_list = data_of_this_point[phase_num_sub * 4 * 2 + phase_num_sub * 3 * 2 : phase_num_sub * 4 * 2 + phase_num_sub * 2 * 3]

                                    self.current_diff_mean_list = [
                                        int(
                                            self.current_diff_mean_str_list[i + 1] + self.current_diff_mean_str_list[i], 16)
                                        for i in range(0, phase_num_sub * 2, 2)]
                                   
                                    # 计算电阻值

                                    current_mean = self.current_mean_list
                                    voltage_mean = self.voltage_mean_list
                                    current_diff = self.current_diff_mean_list

                                    self.resistance_list.append([(voltage_mean[j-1]*current_diff[j] - voltage_mean[j]*current_diff[j-1])/
                                                                                    (current_mean[j-1]*current_diff[j] - current_mean[j]*current_diff[j-1])
                                                                                    for j in range(1,len(current_mean))])
                                ...
                                # 取出位移的平均值
                                self.distance_mean_str_list = self.msg_list_sub[-4*self.halfwave_num:]
                                self.distance_list = [
                                ...
                                try:
                                    ...
                                    # self.udp_socket.sendto(str('aaa'*20).encode('utf-8'), ('192.168.8.120', 1001))
                                    print('ready')
                                except Exception as e:
                                    print(e)
                            ##将数据存入数据库
    ...
    def write_msg(self):
        ...
        self.pw.plot(range(self.halfwave_num), self.Z_score(sum(self.current_rms_list,[])), name='电流', pen=pg.mkPen(color='r')
        ...
        self.pw.plot(range(self.halfwave_num), self.Z_score(sum(self.voltage_rms_list,[])), name='电压', pen=pg.mkPen(color='b')
        ...
        self.pw.plot(range(self.halfwave_num), self.Z_score(sum(self.resistance_list,[])), name='电阻', pen=pg.mkPen(color='k')
