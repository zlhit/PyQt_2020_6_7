...
class Mysql_Search:
    ...
    def mysql_server_concurrency(self):
        ...
        #根据用户输入时间区间查找数据库
        self.cs1.execute("select * from {}__{} where time between '{}' and '{}'".format(datetime.datetime.now().year,
    ...
    def mysql_last_point(self):
        ...
        self.cs1.execute('use resistance')
        self.cs1.execute(
            "select max(id) from {}__{}".format(datetime.datetime.now().year, datetime.datetime.now().month))
        self.last_num = self.cs1.fetchall()
        self.cs1.execute(
            "select * from {}__{} where id = {}".format(datetime.datetime.now().year, datetime.datetime.now().month,
                                                       self.last_num[0][0]))
        # self.cs1.execute('select * from 2020_7')
        #取出查找到的数据
        ...
        F_data = [int(last_data_R) for last_data_R in self.last_data[0][20].split(',')]

        #将数据绘图
        self.pw3.plot(range(len(R_data)), R_data, name='电阻曲线', pen=pg.mkPen(color='r')
                      # , symbol='s', symbolSize=10, symbolBrush=('r')
                      )
        self.pw4.plot(range(len(D_data)), self.Z_score(D_data), name='位移曲线', pen=pg.mkPen(color='b')
                      # , symbol='+', symbolSize=10,symbolBrush=('b')
                      )
        self.pw4.plot(range(len(F_data)), self.Z_score(F_data), name='压力曲线', pen=pg.mkPen(color='k')
                      # , symbol='+', symbolSize=10,symbolBrush=('b')
                      )
        #改变按钮的状态
    ...
    def mysql_previous_point(self):
        ...
        self.pw4.plot(range(len(F_data)), self.Z_score(F_data), name='压力曲线', pen=pg.mkPen(color='r')
                      # , symbol='+', symbolSize=10,symbolBrush=('b')
                      )
