...
class character_data:
    ...
    def tcp_server_start(self):
        ...
        # 建立TCP连接
        self.tcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)#AF_INET局域网通信协议，SOCK_STREAMtcp协议
        ...
        try:
            ...
            self.tcp_socket.bind((self.ip, self.port))#bind捆绑，ip和端口号绑定
            print('绑定端口')
        except:
            pass
        else:
            ...
            # 开一个tcp采集数据的子线程
            self.sever_th = threading.Thread(target=self.tcp_server_concurrency)#concurrency并行
            self.sever_th.start()
                ...
                try:
                    ...
                    if self.recv_msg:
                       ...
                       # print(len(self.msg_list))
                        ...
                        if len(self.msg_list) >= 4:
                            if self.msg_list[0]+ self.msg_list[1]+self.msg_list[2] == 'aabbcc':
                                # 取出半波数
                                # self.msg_list = []
                                self.halfwave_num = int(self.msg_list[3], 16)
                            ...
                            # ##第一次拿到
                            # else:
                            self.halfwave_num = 28
                        # print(len(self.msg_list))
                        if len(self.msg_list) > self.halfwave_num * 34 + 6 + 1:
                            self.msg_list = []
                        # print(len(self.msg_list))
                        # 如果收集到的数达到指定数量，开始继续处理
                        if len(self.msg_list) == self.halfwave_num * 34 + 6:
                            ...
                            try:
                                ...
                                self.distance_list = [
                                    ...
                                    for i in range(0, self.halfwave_num * 4, 4)]
                                #取出压力曲线
                                self.force_mean_str_list = self.msg_list_sub[
                                                              1 + 2 * self.halfwave_num + 2 * 8 * self.halfwave_num + 4 * self.halfwave_num * 3:1 + 2 * self.halfwave_num + 8 * self.halfwave_num * 2 + 4 * self.halfwave_num * 4]

                                self.force_list = [
                                    int(self.force_mean_str_list[i + 3] + self.force_mean_str_list[i + 2] +
                                        self.force_mean_str_list[i + 1] + self.force_mean_str_list[i], 16)
                                    for i in range(0, self.halfwave_num * 4, 4)]
                                ...
                                try:
                                    ...
                                    if self.cs2.fetchall():
                                        ...
                                        sql_create_table = "create table {}__{} (ID int auto_increment primary key,time datetime, preict_result varchar(10), welding_machine varchar(10),welding_material varchar(20)," \
                                                           ...
                                                           "weld2ratio varchar(10),weld3time varchar(10),weld3current varchar(10), weld3ratio varchar(10),distance varchar(200),resistance varchar(200),forcelist varchar(200)," \
                                    ...
                                    sql_ = "insert into {} values(0,'{}','{}','{}','{}','{}','1','{}','{}','{}','{}','{}',null,null,null,null,null,null,'{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}')".format(
                                        ...
                                        int(np.mean(self.current_rms_list) / 1e1), round(self.current_ratio, 2),
                                        str(self.distance_list[:-4])[1:-1],str(self.resistance_list)[1:-1],str(self.force_list[:-4])[1:-1],
    ...
    def write_msg(self):
        ...
        self.pw_dis.plot(range(self.halfwave_num), self.Z_score(self.distance_list), name='位移', pen=pg.mkPen(color='r')
                         # , symbol='s', symbolSize=10, symbolBrush=('r')
                         )
        self.pw_dis.plot(range(self.halfwave_num), self.Z_score(self.force_list), name='压力', pen=pg.mkPen(color='k')
                         # , symbol='s', symbolSize=10, symbolBrush=('r')
                         )
